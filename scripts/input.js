...
miniGame.input = (function() {
	...
	function Keyboard() {
		...
		function keyPress(e) {			
			if(that.pressedKey[e.key] != e.timeStamp){
				that.pressedKey[e.key] = e.timeStamp;
			}
			else{ 
				delete that.pressedKey[e.key];
			 }
		}
		...
		that.update = function (elapsedTime) {
			for (let key in that.pressedKey) {
				if (that.pressedKey.hasOwnProperty(key)) {
